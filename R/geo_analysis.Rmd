...
Is the mean bed occupancy in winter (Q4 + Q1) significantly greater than in summer (Q2 + Q3)?

```{r}
total_avg_acute_occupancy %>% 
  separate(quarter, into = c("year", "quarter"), sep = "Q") %>% 
  mutate(season = if_else(quarter == 1 | quarter == 4, "winter", "summer"), .after = "quarter") %>% 
  filter(year <= 2019) %>% 
  ggplot() +
...
Null hypothesis H0: Mean acute bed occupancy is the same in the summer months as 
in the winter months;
$$
H_0: μ_{winter} − μ_{summer} = 0
$$

Alternative hypothesis H1: Mean acute bed occupancy is greater in winter than in 
summer;
$$
H_1: μ_{winter} − μ_{summer} > 0
$$
