...
all_range_plot <- waiting_time_all_range %>% 
  group_by(year_quarter) %>% 
  ggplot()+
...
The H0 is rejected. There is no evidence proof that the mean attendances after Covid is the same as the mean attendances before Covid.

Since this is a right-sided test, we can confirm that the mean attendances after Covid is greater than the mean attendance before Covid.
...
ae_waiting_time_clean_long_p %>% 
  group_by(year_quarter) %>% 
  ggplot()+
  aes(x = year_quarter, y = sum(number_of_attendances_aggregate), group = region, color = region)+
  geom_line()+
  geom_point()
...
The datasets in this report is open data that authorized under NHS Scotland's protection. We do not own the copyright of all datasets. The material contained in this report is distributed without profit for research and educational purposes. 

The dataset recorded and collected the information of every episodes accessing the hospital services from the year of 2007 to 2022, which may not consent by patients. The data is anonymous, as well as contains without sensitive personal details such as address, occupancy and the reasons for accessing hospital services. However, there maybe a concern that the third party access the data for business usage. 

The data may have bias since it is not standardized for each units of the hospital service. For instances some units are not able to provide a full record due to the limitation of resources, which may cause difference to the result.
