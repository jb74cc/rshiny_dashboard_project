...
Health board codes are not easy to understand - join in health board code definitions;
...
```{r}
# hb_joined <- left_join(beds_by_hb_trim, )
```
...
beds_by_hb_trim %>%
  ggplot() +
  aes(x = quarter, y = hb_mean_occupancy, colour = hb, group = hb) +
  geom_line() +
  theme_bw() +
