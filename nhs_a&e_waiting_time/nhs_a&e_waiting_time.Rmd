...
ae_waiting_time_clean_long_p %>% 
  group_by(year_quarter) %>% 
  filter(hrs_waiting) %>% 
  summarise(total_attendances_4rs = sum(number_attendances),
            total_attendances_8hr = sum())
  ggplot(aes(x = year_quarter, y = number_attendances, group_by = hrs_waiting))+
  geom_line()+
  geom_point()
  
ae_waiting_time_clean_long_p %>% 
  ggplot(aes(x = year_quarter, y = number_attendances, group = hrs_waiting))+
  geom_line()+
  geom_point()
  
  
# ggplot(ae_waiting_time_clean_long_p, aes(year_quarter, colour = number_attendances,, stat = "count")) +geom_freqpoly(binwidth = 500)
