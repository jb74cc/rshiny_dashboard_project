...
region_waiting_time <- ae_waiting_time_clean_long_p %>% 
  ...
  summarise(attendances = sum(number_of_attendances_aggregate))

region_waiting_time %>% 
  group_by(year_quarter) %>% 
  ggplot()+
  aes(x = year_quarter, y = log10(attendances), group = region, color = region)+
  geom_line()+
  geom_point()+
  xlab("Year(Quarter)")+ylab("Number of Attendances (log 10)")+
  theme(axis.text.x = element_text(angle = 90, size = 5, vjust = 0.5, hjust=1))+
  labs(color="Time Waiting", size = 3)+
  ggtitle("Average Waiting Time of Every Region")
